...
def wct_twostyles(alpha, cf, s0f, s1f=None, beta=None):
    def decomposition(f, regularize=True):
        ...
        if regularize:
            f_covariance = torch.mm(fv, fv.t()).div((f_width * f_height) - 1) + torch.eye(
                f_channels).double()  # construct regularized covariance matrix
        else:
...
def wct_interpolation(alpha, cf, sf, s1f=None, beta=None):
...
def wct_mask(alpha, cf, sf):
    ...
    s_k = sf_sizes[0] # same number of channels ad content features
